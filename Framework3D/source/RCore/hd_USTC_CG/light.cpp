...
Color Hd_USTC_CG_Rect_Light::Sample(
    ...
    const std::function<float()>& uniform_float)
{
    auto sampledX = (-0.5 + uniform_float()) * width;
    auto sampledY = (-0.5 + uniform_float()) * height;
    auto transform = Get(HdTokens->transform).GetWithDefault<GfMatrix4d>();
    sampled_light_pos = transform.TransformAffine(GfVec3f(sampledX, sampledY, 0)) + corner0;

    dir = (sampled_light_pos - pos).GetNormalized();

    float cosVal = GfDot(GfCross(corner2 - corner0, corner1 - corner0), dir);
    sample_light_pdf = 1 / (width * height * cosVal);
...
{
    double distance;
    if (ray.Intersect(corner0, corner2, corner1, &distance)){
        depth = distance;
        return irradiance / M_PI;
    }
    if (ray.Intersect(corner1, corner2, corner3, &distance)) {
        depth = distance;
        return irradiance / M_PI;
    }
    depth = std::numeric_limits<float>::infinity();
