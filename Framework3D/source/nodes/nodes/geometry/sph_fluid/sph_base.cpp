...
void SPHBase::compute_non_pressure_acceleration()
{
    ...
    for (int i = 0; i < ps_.particles().size(); ++i) {
        ...
        // 
        //}
        p->acceleration_non() = gravity_;
        for (auto& q : p->neighbors()) {
            p->acceleration_non() += compute_viscosity_acceleration(p, q);
        }
    }
}
...
{
    Vector3d v_ij = p->vel() - q->vel();
    Vector3d x_ij = p->x() - q->x();
    Vector3d grad = grad_W(p->x() - q->x(), ps_.h());
    double mass = ps_.mass();

    Vector3d laplace_v = 10 * mass / q->density() * v_ij.dot(x_ij) /
...
void SPHBase::compute_pressure_gradient_acceleration()
{
    ...
    for (int i = 0; i < ps_.particles().size(); ++i) {
        auto& p = ps_.particles()[i];
        // (HW TODO) Traverse all particles and compute each particle's acceleration from pressure
        // gradient force
        double mass = ps_.mass();
        p->acceleration_p().setZero();
        for (auto& q : p->neighbors()) {
            Vector3d grad = grad_W(p->x() - q->x(), ps_.h());
            p->acceleration_p() -= mass *
...
void SPHBase::advect()
{
    ...
    for (int i = 0; i < ps_.particles().size(); ++i) {
        ...
        // Your code here 
        p->vel() += (p->acceleration_non() + p->acceleration_p()) * dt_;
        p->x() += p->vel() * dt_;
        check_collision(p);
        // ---------------------------------------------------------
